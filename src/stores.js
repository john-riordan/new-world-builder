...
export const shareLink = derived(
  ...
  ([$weps, $wep0tree0, $wep0tree1, $wep1tree0, $wep1tree1]) => {
    ...
    if ($wep0tree0.list.length) {
      params.w0t0 = $wep0tree0.list;
    }
    if ($wep0tree1.list.length) {
      params.w0t1 = $wep0tree1.list;
    }
    if ($wep1tree0.list.length) {
      params.w1t0 = $wep1tree0.list;
    }
    if ($wep1tree1.list.length) {
      params.w1t1 = $wep1tree1.list;
    }

    const encodedParams = btoa(JSON.stringify(params));

    return `?share=${new URLSearchParams(encodedParams).toString()}`;
  }
);
