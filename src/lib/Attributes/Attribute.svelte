<script>
	import { availableAtrPts } from '../../stores';
...
<section class="container" class:inactive={$pts === MIN_ATTR_PTS}>
	<div class="left">
		<button on:click={sub1}>
		...
		<h2 class="attribute-value">{$pts}</h2>
		<button on:click={add10}>
			<svg width="26" height="30" viewBox="0 0 26 30">
				<path
					fill-rule="evenodd"
					clip-rule="evenodd"
	...
	<div class="right">
		<h4 class="attribute-name">{name}</h4>
...
<style>
	...
	button svg {
		display: block;
		filter: drop-shadow(0px 3px 8px var(--black));
	}
	...
	.bar {
		...
		overflow: hidden;
		border: 2px solid hsla(120, 3%, 21%, 1);
	}
	.bar-fill {
		position: absolute;
		top: 0;
